...
async def tutor(bot,update):
        ...
        tutup = InlineKeyboardMarkup([[InlineKeyboardButton("TUTUP", callback_data="tutup")]])
        await update.message.edit(text = ttutor, reply_markup = tutup)
...
async def xkembali(bot,update):
        await update.message.edit(f"""**Mau lu apain nih file?**\n\n**Nama file** :- {filename}\n**Ukuran file** :- {humanize.naturalsize(file.file_size)}\n**Dc ID** :- {dcid}""", reply_to_message_id=message.id, reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("UBAH NAMA📝", callback_data="rename")],
                    [InlineKeyboardButton("ENCODE RESOLISI♻", callback_data='encode')],
                    [InlineKeyboardButton("POTONG DURASI VIDEO✂", callback_data='trim')],
                    [InlineKeyboardButton("EXTRAC AUDIO🎼", callback_data='extmp3')],
...
@Client.on_callback_query(filters.regex('encode'))
async def encode(bot,update):
        tencode = "**COMINGSOON\n\nTUNGGU UPDATE SELANJUTNYA TOD💩**"
        encodebutton = InlineKeyboardMarkup([[InlineKeyboardButton("KEMBALI", callback_data="xkembali")]])
        await update.message.edit(text = tencode, reply_markup = encodebutton)
        
@Client.on_callback_query(filters.regex('trim'))
async def encode(bot,update):
        ttrim= "**COMINGSOON\n\nTUNGGU UPDATE SELANJUTNYA TOD💩**"
        trimbutton = InlineKeyboardMarkup([[InlineKeyboardButton("KEMBALI", callback_data="xkembali")]])
        await update.message.edit(text = ttrim, reply_markup = trimbutton)
        
@Client.on_callback_query(filters.regex('extmp3'))
async def encode(bot,update):
        textmp3 = "**COMINGSOON\n\nTUNGGU UPDATE SELANJUTNYA TOD💩**"
        extmp3button = InlineKeyboardMarkup([[InlineKeyboardButton("KEMBALI", callback_data="xkembali")]])
        await update.message.edit(text = textmp3, reply_markup = extmp3button)
        
@Client.on_callback_query(filters.regex('addthumb'))
async def encode(bot,update):
        taddthumb = "**KIRIM FOTO/THUMBNAIL DISINI⚠\n\nFOTO AKAN OTOMATIS JADI THUMBNAIL PERMANEN,KALO MAU DIGANTI KLIK** /hapusthumbnail **BUAT HAPUS THUMBNAIL,TRUS LU KIRIM LAGI THUMBNAIL BARU😪\n\nKALO UDAH LU KIRIM TINGGAL LU UBAH NAMA/RENAME NTAR THUMBNAIL LANGSUNG KEPASANG DIVIDEO LU🖕\n\nKALO GAPAHAM CHAT ADMIN YG GANTENG DIBAWAH INI😗**"
        addthumbbutton = InlineKeyboardMarkup([
                                              [InlineKeyboardButton("ADMIN👤", url="https://t.me/MSDZULQURNAIN")],
                                              [InlineKeyboardButton("KEMBALI", callback_data="xkembali")]])
