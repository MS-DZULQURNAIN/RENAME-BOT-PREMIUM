...
async def rename(bot, update):
    ...
    await update.message.delete()
    await update.message.reply_text(f"__Kirim nama baru buat file dengan reply pesan ini...__", reply_to_message_id=id,
...
async def pasangthumbnail(bot, update):
        id = update.message.reply_to_message_id
        await update.message.reply_text(f"__KIRIM FOTO LU BUAT THUMBNAIL__", reply_to_message_id=id)
...
async def doc(bot, update):
    ...
    ms = await update.message.edit("__LAGI DOWNLOAD,SABAR TOD...__")
    used_limit(update.from_user.id, file.file_size)
    c_time = time.time()
    total_used = used + int(file.file_size)
    used_limit(update.from_user.id, total_used)
    try:
        path = await bot.download_media(message=file, progress=progress_for_pyrogram, progress_args=("__LAGI DOWNLOAD,SABAR TOD...__",  ms, c_time))
    ...
    if value < file.file_size:
        await ms.edit("__UPLOAD KE TELE, SABAR TOD...__")
        try:
            filw = await app.send_document(log_channel, document=file_path, thumb=ph_path, caption=caption, progress=progress_for_pyrogram, progress_args=("__UPLOAD KE TELE, SABAR TOD...__",  ms, c_time))
        ...
        c_time = time.time()
        try:
            await bot.send_document(update.from_user.id, document=file_path, thumb=ph_path, caption=caption, progress=progress_for_pyrogram, progress_args=("__UPLOAD KE TELE, SABAR TOD...__",  ms, c_time))
            await ms.delete()
            os.remove(file_path)
        except Exception as e:
            neg_used = used - int(file.file_size)
...
async def vid(bot, update):
    ...
    if value < file.file_size:
        await ms.edit("__UPLOAD KE TELE, SABAR TOD...__")
        try:
            filw = await app.send_video(log_channel, video=file_path, thumb=ph_path, duration=duration, caption=caption, progress=progress_for_pyrogram, progress_args=("__UPLOAD KE TELE, SABAR TOD...__",  ms, c_time))
            from_chat = filw.chat.id
            mg_id = filw.id
            time.sleep(2)
            await bot.copy_message(update.from_user.id, from_chat, mg_id)
        ...
        c_time = time.time()
        try:
            await bot.send_video(update.from_user.id, video=file_path, thumb=ph_path, duration=duration, caption=caption, progress=progress_for_pyrogram, progress_args=("__UPLOAD KE TELE, SABAR TOD...__",  ms, c_time))
            await ms.delete()
            os.remove(file_path)
        except Exception as e:
            neg_used = used - int(file.file_size)
...
async def aud(bot, update):
    ...
    if thumb:
        ...
        c_time = time.time()
        try:
            await bot.send_audio(update.message.chat.id, audio=file_path, caption=caption, thumb=ph_path, duration=duration, progress=progress_for_pyrogram, progress_args=("__UPLOAD KE TELE, SABAR TOD...__",  ms, c_time))
            await ms.delete()
            os.remove(file_path)
            os.remove(ph_path)
        except Exception as e:
        ...
        c_time = time.time()
        try:
            await bot.send_audio(update.message.chat.id, audio=file_path, caption=caption, duration=duration, progress=progress_for_pyrogram, progress_args=("__UPLOAD KE TELE, SABAR TOD...__",  ms, c_time))
            await ms.delete()
            os.remove(file_path)
        except Exception as e:
            await ms.edit(e)
